...
`$HOME/.openclaw/nibvok-ai-security-audit.log` — one JSON line per decision (`product` is a field, so the file stays valid JSONL):
...
Three suites, runnable without a Gateway:
